...
The model has `r 100*confusionMatrix(testing$classe,pred)$overall[1]`% accuracy on the testing set and an expected out-of-sample error rate of `r 100 - 100*confusionMatrix(testing$classe,pred)$overall[1]`%.
...
The model has `r 100*confusionMatrix(testing$classe,pred)$overall[1]`% accuracy on the testing set and an expected out-of-sample error rate of `r 100 - 100*confusionMatrix(testing$classe,pred)$overall[1]`%. Since the accuracy is higher, we will ultimately use the model created from the random forest algorithm with cross-validation.
